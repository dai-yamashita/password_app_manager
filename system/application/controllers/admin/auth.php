...
class Auth extends Controller {
	...
	function login() {
		...
		if ($val->run() AND $this->dx_auth->login($val->set_value('username'), $val->set_value('password'), $val->set_value('remember'))) {
			...
			if ($this->dx_auth->is_banned()) {
				...
				$data['flash']['defaulterror'] = $this->form_validation->error_string();
    ...
    function id_check($id) {
        ...
        if (empty($rs)) {
            $this->form_validation->set_message('id_check', 'Invalid password.');
            return FALSE;
        }
        return TRUE;
    }
    ...
    function captcha_check($code) {
        ...
        if ($use_captcha == 'yes' ) {
            if ($this->dx_auth->is_captcha_expired()) {
                ...
                $this->form_validation->set_message('captcha_check', 'Your confirmation code has expired.<br />Please try again.');
                $result = FALSE;
            }
            elseif ( ! $this->dx_auth->is_captcha_match($code)) {
                $this->form_validation->set_message('captcha_check', 'Your confirmation code does not match the one in the image.<br />Please try again.');
                $result = FALSE;
            }
    ...
    function deny() {
        die('Sorry, you are not allowed to access in this page.');
    }
