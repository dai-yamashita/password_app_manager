<div class="title title-spacing">
    <h2>Import groups</h2>
    <span style="color:#000; font-size:14px">Import groups in <b><?= $importfmt?></b> format.</span> <br /><br />       
	<?php if ($importfmt == 'csv') { ?>    
	<a href="<?= base_url() ?>uploads/sample/sample-groups.csv" >Click here to Download sample <?= $importfmt?> format &raquo;.</a>
    <?php } elseif ($importfmt == 'xml') { ?>
	<a href="<?= base_url() ?>uploads/sample/sample-groups.xml" >Click here to Download sample <?= $importfmt?> format &raquo;.</a>    
    <?php } ?>
    
</div>
<?php !empty($flash) ? flash($flash) : '' ?>

<form method="post" action="<?= site_url('admin/department/import_step/csv/step/1') ?>"  id="exportfrm"  enctype="multipart/form-data" >
<?php if ( $importfmt == 'csv') { ?>
<div class="title title-spacing">
<h2>Step 1</h2>
</div>
