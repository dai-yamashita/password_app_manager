...
?>
<form action="<?php echo site_url('admin/user/form') ?>" method="post" >
<ul>
    <li>
...
<?php if ( ! $this->dx_auth->is_role( array('owner', 'administrator', 'manager') ) || empty($user_id) ) { ?> 
    <li>
...
<?php endforeach; ?>    
    ...
    <!--<li>
		<label class="desc">Select Group</label>
      	<div>
      	  <select tabindex="3" class="field select small" name="deptid" >
...
<label class="desc">Assign user to group access. </label>
<div class="peopleaccess" >
<ul>
<?php
$tmp = array();
if (empty($usergroups)) $usergroups = array();
foreach($usergroups as $k => $v) $tmp[] = $v['deptid'];
foreach($department as $k => $v ) { 
$gidlist[] = $v['deptid'];
$id = $v['deptid'];
?>
<li style="padding:3px 0" ><label><input type="checkbox" name="user_groups[]" value="<?= $id ?>" <?= in_array($id, $tmp) ? 'checked=checked' : '' ?>  /><?= $v['groupname'] ?></label></li>
<?php } ?>
</ul>
</div>
