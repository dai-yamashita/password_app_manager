<div class="title title-spacing">
    <h2>Import users</h2>
    <span style="color:#000; font-size:14px">Import users in <b><?= $importfmt?></b> format.</span><br /><br />
	<?php if ($importfmt == 'csv') { ?>    
	<a href="<?= base_url() ?>uploads/sample/sample-users.csv" >Click here to Download sample <?= $importfmt?> format &raquo;.</a>
    <?php } elseif ($importfmt == 'xml') { ?>
	<a href="<?= base_url() ?>uploads/sample/sample-users.xml" >Click here to Download sample <?= $importfmt?> format &raquo;.</a>    
    <?php } ?>
    
</div>
<?php !empty($flash) ? flash($flash) : '' ?>

<form method="post" action="<?= site_url("admin/user/import/$importfmt") ?>"  id="exportfrm"  enctype="multipart/form-data" >
<?php if ( $importfmt == 'csv') { ?>
<input type="file" name="userfile" class="field text large"   />
 <table width="99%" border="1" class="csvoptable" >
  ...
  <tr>
    <td>&nbsp;</td>
    <td><input type="submit" name="btnimport2" value="Import"  /></td>
  </tr>
</table>
<?php } else { ?>
<input type="file" name="userfile" class="field text large"   />
