...
<?php echo (!empty($results['domain_id'])) ? '<h2>Edit domain access</h2>' :  '<h2>Add domain access</h2>' ?>    
</div>
...
if (isset($uri['project_id']) && !empty($uri['project_id'])) {
	$domainname 	= set_value('domainname');	
	$domainname		= !empty($domainname) ? $domainname : (isset($uri['project_id']) ? $uri['project_id'] : '');	
} else {
	$domainname 	= set_value('domainname');
...
?>
<form action="<?php echo site_url( "admin/domain/form/$domain_id") ?>" method="post" >
<ul>
...
<h2 style="margin:0">Define custom login fields for this domain</h2>
</div>
...
<thead> 
  <tr>
    ...
    <th width="77%" ><div align="left">&nbsp;&nbsp;Value</div></th>
    <th width="77%">&nbsp;</th>
    </tr>
</thead>  
<?php
...
?>
  <tr class="<?= (++$alt%2 == 0) ?  'alt' : '' ?>" >
    ...
    <td>
<a class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Delete this access" href="<?php echo site_url("admin/domain/delete_customfield/$customfieldid") ?>" onclick="return confirm('Are you sure you want to delete?')" >
...
<?php } ?>
    ...
    <input type="button" value="&laquo;Back" class="submit" onclick="document.location.href='<?= site_url('admin/domain/browse')?>'" />
    </li>
</ul>
